...
A practical, step-by-step framework for planning and executing module refactors safely — how to sequence changes to minimize risk, and what to test at each stage.

## Why This Exists

Refactors go wrong most often not because the new code is bad, but because changes are sequenced poorly: behavior and structure get changed at the same time, hidden callers get broken, or "the tests passed" gets trusted too much. This framework lays out an order of operations designed to keep every step small, revertible, and verifiable — so you always know exactly what broke if something does.

## Who This Is For

Engineers planning a refactor of an existing module, service, or class — especially in codebases with real production usage, multiple callers, or thin test coverage where a "just rewrite it" approach is too risky.
...
- Use it as a **checklist** when planning a refactor PR or design doc.
- Copy the sequencing table into your PR description and check off stages as you go.
- Adapt the "what to test" column to your stack's testing tools (unit, integration, property-based, etc.).
...
[Apache](LICENSE) — feel free to use and adapt this framework.
